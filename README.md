...
| 継続利用機能     | 一度きりの入力が中心／再来時の引き継ぎが難しい                                                 | 再来時も前回の情報をもとに提案できる履歴機能（本リリース想定）                                    |
 ...
 * 再来ユーザー対応
   ...
   * 前回の内容のベースから再カウンセリング始められることで、再入力の手間を削減し、リピーター対応に役立ちます。
...
* 主な機能：
    * 固定された質問リストのチャットUI: 事前に定義された質問リストを順番に表示し、ユーザーはそれぞれの質問に回答できます。
    * 疑似チャットUI（form_with + JS で非同期送信）: ユーザーの入力を非同期で処理し、チャット形式で表示します。
